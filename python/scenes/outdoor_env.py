...
def main() -> int:
    out = sys.argv[1] if len(sys.argv) > 1 else "outdoor_env.avif"
    ...
    light_mat = scene.add_material(
        lc.Material(base_color=(0.92, 0.90, 0.85), roughness=0.9, emission=(12, 11, 9))
    )
    scene.add_mesh(lc.make_quad(light_corner, light_u, light_v, light_mat))
    scene.add_quad_light(light_corner, light_u, light_v, (12, 11, 9), use_mis=True)
