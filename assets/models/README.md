...
- `spot_triangulated.obj` / `spot_texture.png` — **Spot** (~5.9k triangles), Keenan Crane’s cow from the [CMU 3D Model Repository](https://www.cs.cmu.edu/~kmcrane/Projects/ModelRepository/). Triangulated mesh + 1024² albedo; no `.mtl` (bind texture via `Material.albedo_tex` in scenes). Used in the `atelier` gallery showcase (`python/scenes/atelier.py`).
