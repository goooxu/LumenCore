...
cfg = lc.RenderConfig(width=512, height=512, spp=64, denoise=True, output_path="out.heic")
lc.Renderer().render(scene, cam, cfg)
```

绑定在 `bindings/lumencore_module.cpp`：把 C++ `Scene` / `Material` / `PhysXWorld` 等暴露给 Python。

首页 Gallery：`atelier` 综合展示 + 两封面（`dusk_observatory` / `assembly_hall`）+ `gallery_compare` 特性 ON/OFF；下面各节仍用旧 `outputs/*.heic` 单图做章节对照。
...
脚本：`python/scenes/atelier.py` → `outputs/gallery/showcase.heic`。PhysX 短仿真定格 + IAS，火焰 / HDRI / NEE / GGX / 法线 / Spot / Beer-Lambert 同框。

### 暮潮观测站 / Assembly Hall — 封面

- `python/scenes/dusk_observatory.py` → `outputs/gallery/dusk_observatory.heic`：海岸暮色平台，多材质光学台 + Beer 潮池 + 火焰信标。
- `python/scenes/assembly_hall.py` → `outputs/gallery/assembly_hall.heic`：工厂正午 HDR、磨砂透射炉火、吸收烟柱、PhysX 倾泻 Spot、糖果 Sparky 与镂空齿轮。
