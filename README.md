...
- **Dual render backends (migration)** — `RenderConfig.backend = "optix"` (default, full OptiX path tracer) or `"vulkan"` (Phase 2b: Vulkan RT — GGX + glass + quad/spot/HDRI NEE + MIS + albedo/normal maps; no denoiser/volumes/IAS yet)
