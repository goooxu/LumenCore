...
# PhysX GPU .so: NRTX_PHYSX_ROOT override, else CMake build-tree install.
if [[ -n "${NRTX_PHYSX_ROOT:-}" ]]; then
  PHYSX_ROOT_HOST="${NRTX_PHYSX_ROOT}"
else
  PHYSX_ROOT_HOST="${BUILD_DIR}/_deps/physx"
fi

PHYSX_MOUNT_ARGS=()
PHYSX_BIN_IN_CT=""
if [[ -d "${PHYSX_ROOT_HOST}/bin" || -d "${PHYSX_ROOT_HOST}" ]]; then
  PHYSX_MOUNT_ARGS+=(-v "${PHYSX_ROOT_HOST}:/physx:ro")
  PHYSX_BIN_IN_CT="/physx/bin"
fi
# Prefer explicit mount, then in-container build tree path after first cmake.
LD_EXTRA="${HOST_LIB_DIR}"
if [[ -n "${PHYSX_BIN_IN_CT}" ]]; then
  LD_EXTRA="${PHYSX_BIN_IN_CT}:${LD_EXTRA}"
fi
LD_EXTRA="/out/_deps/physx/bin:${LD_EXTRA}"
...
if [[ -z "${RTCORE_VER}" ]]; then
  echo "error: libnvidia-rtcore.so.* not found on host (NVIDIA driver install incomplete)" >&2
  exit 1
fi
NVOPTIX_VER=""
for d in "${LIB_DIRS[@]}"; do
...
# Host PhysXGpu prebuilts (read-only). Writable packman cache lives under the build dir.
PACKMAN_MOUNT_ARGS=()
if [[ -d /tmp/packman-cache/chk/PhysXGpu ]] && [[ -r /tmp/packman-cache/chk/PhysXGpu ]]; then
  PACKMAN_MOUNT_ARGS+=(-v /tmp/packman-cache/chk/PhysXGpu:/packman-physxgpu:ro)
fi
